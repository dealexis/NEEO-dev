...
module.exports = function controller(driver) {
    ...
    this.addListener = function (params) {
        ...
        let listIndent = self.listeners.findIndex((listen) => {
            return listen.command == params.command
        });
    ...
    this.addImageHelper = function (deviceId, imageName, listened) {//function called by the MetaDriver to store
        const indent = self.imageH.findIndex((elt) => {
            return (elt.name == imageName && elt.deviceId == deviceId);
        });
    ...
    this.addLabelHelper = function (deviceId, labelName, listened, actionListened) {//function called by the MetaDriver to store
        ...
        if (indent < 0) { //add only if not existant
            ...
            return self.labelH[indent];
        }
    };

    this.addSensorHelper = function (deviceId, sensorName, listened) {//function called by the MetaDriver to store
        const indent = self.sensorH.findIndex((elt) => {
            return (elt.name == sensorName && elt.deviceId == deviceId);
        });
        if (indent < 0) { //add only if not existant
            ...
            return self.sensorH[indent];
        }
    };

    this.addSwitchHelper = function (deviceId, switchName, listen, evaldo) {//function called by the MetaDriver to store
        const indent = self.switchH.findIndex((elt) => {
            return (elt.name == switchName && elt.deviceId == deviceId);
        });
        if (indent < 0) { //add only if not existant
    ...
    this.evalWrite = function (evalwrite, result, deviceId) {
        if (evalwrite) { //case we want to write inside a variable
            evalwrite.forEach(evalW => {
                if (evalW.deviceId) {
                    deviceId = evalW.deviceId
                } //this is specific for listeners and discovery, when one command should be refreshing data of multiple devices (example hue bulbs)
    ...
    this.evalDo = function (evaldo, result, deviceId) {
        if (evaldo) { //case we want to trigger a button
            evaldo.forEach(evalD => {
                if (evalD.test == '' || evalD.test == true) {
                    evalD.test = true;
                } //in case of no test, go to the do function
    ...
    this.getConnection = function (commandtype) {
        return self.connectionH[self.connectionH.findIndex((item) => {
            return (item.name == commandtype);
        })];
    };
    ...
    this.assignProcessor = function (commandtype) {
        ...
        if (commandtype == HTTPGET) {
            ...
            console.log('Error in meta settings: The commandtype to process is not defined: ' + commandtype);
        }
    };
    ...
    this.initiateProcessor = function (commandtype) { // Initiate communication protocoles
        return new Promise(function (resolve, reject) {
            ...
            processingManager.initiate(self.getConnection(commandtype))
                ...
                .catch((err) => {
                    console.log('Error during initiation with commandtype : ' + commandtype);
                    console.log(err);
                    reject(err);
                });
        });
    ...
    this.listenProcessor = function (command, commandtype, listener, deviceId) { // process any command according to the target protocole
        return new Promise(function (resolve, reject) {
            ...
            const params = {
                'command': command,
                'listener': listener,
                '_listenCallback': self.onListenExecute,
    ...
    this.onListenExecute = function (result, listener, deviceId) {
        ...
        self.queryProcessor(result, listener.queryresult, listener.type, deviceId).then((result) => {
            ...
            if (listener.evalwrite) {
                self.evalWrite(listener.evalwrite, result, deviceId);
            }
            if (listener.evaldo) {
                self.evalDo(listener.evaldo, result, deviceId);
            }
        });
    };
    ...
    this.listenStart = function (listener, deviceId) {
        return new Promise(function (resolve, reject) {
            try {
                ...
                reject('Error when starting to listen. ' + err);
            }
        });
    };
    ...
    this.initialise = function (deviceId) {
        self.sliderH.forEach((helper) => {
            helper.initialise(deviceId);
        });//No need to cleanup as double addition is protected
        self.switchH.forEach((helper) => {
            helper.initialise(deviceId);
        });//No need to cleanup as double addition is protected
        self.imageH.forEach((helper) => {
            helper.initialise(deviceId);
        });//No need to cleanup as double addition is protected
        self.labelH.forEach((helper) => {
            helper.initialise(deviceId);
        });//No need to cleanup as double addition is protected
        self.sensorH.forEach((helper) => {
            helper.initialise(deviceId);
        });//No need to cleanup as double addition is protected
    ...
    this.onButtonPressed = function (name, deviceId) {
        ...
        let theButton = self.buttons[self.buttons.findIndex((button) => {
            return button.name == name && button.deviceId == deviceId;
        })];
