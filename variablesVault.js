    ...
    constructor() {
        ...
        this.addVariable = function (name, value, deviceId, persisted) {
            return new Promise(function (resolve, reject) {
                ...
                if (self.variables.findIndex((elt) => {
                    return elt.name == internalVariableName
                }) < 0) {//the variable is new
                    self.variables.push({
        ...
        */
        this.addObserver = function (name, theFunction, deviceId, componentRegistering) { // who listen to variable changes.
            ...
            try {
                ...
                if (name != undefined && name != '' && theFunction != undefined && theFunction) {
                    let observersList = self.variables.find(elt => {
                        return elt.name == internalVariableName
                    }).observers;
                    if (observersList.findIndex(func => {
                        return (func.observer == componentRegistering)
                    }) < 0) {//to avoid adding multiple times an oberver
        ...
        this.getValue = function (name, deviceId) {
            let internalVariableName = toInternalName(name, deviceId);
            let indexRes = self.variables.findIndex(elt => {
                return elt.name == internalVariableName
            });
            if (indexRes < 0) {
                return undefined
        ...
        this.writeVariable = function (name, value, deviceId) {//deviceId necessary as push to components.
            let internalVariableName = toInternalName(name, deviceId);
            let foundVar = self.variables.find(elt => {
                return elt.name == internalVariableName
            });
            if (!foundVar) {
                console.log("The variable you are requesting doesn\'t seems to be properly declared.")
            }
            if (foundVar) {
                ...
                console.log("Variable " + name + " with device " + deviceId + " not found. Can't assign value.")
            }
        }
        ...
        this.readVariables = function (inputChain, deviceId) { //replace in the input chain, all the variables found of the same deviceId
            let preparedResult = inputChain,
                initType = typeof preparedResult;
        ...
        this.retrieveValueFromDataStore = function (name, deviceId) {
            return new Promise(function (resolve, reject) {
                ...
                self.getDataFromDataStore().then((store) => {
                    if (store) {
                        let valueIndex = store.findIndex((key) => {
                            return key.name == internalVariableName
                        });
                        if (valueIndex >= 0) {
                            resolve(store[store.findIndex((key) => {
                                return key.name == internalVariableName
                            })].value);
                        } else {
                            resolve(undefined)
