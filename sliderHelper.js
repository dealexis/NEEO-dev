...
class sliderHelper {
    ...
    constructor(deviceId, variableListened, evaldo, slidername, controller) {
        ...
        this.update = function (deviceId, theValue) {
            return new Promise(function (resolve, reject) {
                if (self.value != theValue) {
                    self.value = theValue;
                    controller.sendComponentUpdate({
                        uniqueDeviceId: deviceId,
                        component: self.name,
                        value: Math.round(theValue)
                    })
                        .then((result) => {
                            console.log("update performed : new value : " + theValue + " component " + controller.name + "/" + self.name + "/" + deviceId);
                            console.log(result)
                        })
                        .catch((err) => {
                            console.log("Error while trying to update the value : " + theValue + " in this component : " + controller.name + "/" + deviceId + "/" + self.name + " => " + err);
                            reject(err);
                        });
                }
                resolve();
        ...
        this.set = function (deviceId, theValue) {
            ...
            console.log(chalk.redBright('=== slider set ==='))
            console.log(chalk.redBright(deviceId))
            console.log(chalk.redBright(theValue))
            ...
            return new Promise(function (resolve, reject) {
                ...
                controller.sendComponentUpdate({uniqueDeviceId: deviceId, component: self.name, value: theValue})
                    ...
                    .catch((err) => {
                        console.log("Error while trying to set the value : " + theValue + " in this component : " + controller.name + "/" + deviceId + "/" + self.name + " => " + err);
                    });
        ...
        this.initialise = function (deviceId) {
            ...
            console.log(chalk.redBright('slider initialise'))
            console.log(self.variableListened + self.update + deviceId + self.name)
